...
def helpMessage() {
    ...
    Alignment options
    --max_IntronL [int]             Maximum intron length for star aligner (50000 for c.elegans)
    Presets:
...
max_IntronL = params.max_IntronL
...
if(params.aligner == 'star'){
    ...
    summary['max_IntronL'] = max_IntronL
...
process featureCounts {
    ...
    if (params.singleEnd) {
        """
        samtools sort $bam_featurecounts -o sorted.bam
        featureCounts -t exon -a $gtf -Q 10 -g gene_id -o ${bam_featurecounts.baseName}_gene.featureCounts.txt -s $featureCounts_direction sorted.bam
        featureCounts -t exon -a $gtf -Q 10 -g gene_biotype -o ${bam_featurecounts.baseName}_biotype.featureCounts.txt -s $featureCounts_direction sorted.bam
